// Ashraf1.cpp - First Push (Graphics Enhancement Phase)
// Branch: feature/ashraf-graphics-enhancement-v2
// Contributor: Ashraf
// Push #1 - Enhanced Visual Effects System
...
// [ASHRAF MODIFICATION #1]
// Enhanced Visual Effects with Lighting System
struct Lighting {
    float intensity;
    float ambientLevel;
    bool  enableGlow;
...
// [ASHRAF#1] Lighting system
Lighting gLighting = {1.0f, 0.3f, true};
...
static void col3(float r, float g, float b) { 
    // [ASHRAF#1] Apply lighting to colors
    glColor3f(r * gLighting.intensity, g * gLighting.intensity, b * gLighting.intensity); 
}

static void col4(float r, float g, float b, float a) { 
    glColor4f(r * gLighting.intensity, g * gLighting.intensity, b * gLighting.intensity, a); 
}
...
// [ASHRAF MODIFICATION #1] - Enhanced glow effect renderer
static void drawGlowedCircle(float cx, float cy, float r, float glowIntensity) {
    if(gLighting.enableGlow) {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        
        // Multiple halo layers for glow effect
        col4(1.0f, 1.0f, 1.0f, glowIntensity * 0.3f);
        drawFilledCircle(cx, cy, r * 1.5f, 40);
        
        col4(1.0f, 1.0f, 1.0f, glowIntensity * 0.15f);
        drawFilledCircle(cx, cy, r * 2.0f, 40);
...
static void drawRadar() {
    ...
    for(int i = 0; i < MAX_DRONES; i++){
        if(gDrones[i].active && gDrones[i].detected){
            ...
            if(blipPulse > 0.3f){
                // [ASHRAF#1] Enhanced glowing blip
                col4(1.0f, 0.25f, 0.05f, 1.0f);
                drawGlowedCircle(cx + wx, cy + wy, 4, 0.8f);
            }
        }
    }
...
static void display() {
    ...
    // [ASHRAF#1] Display lighting status
    col3(1.0f, 0.8f, 0.2f);
    drawText(12, 8, "[ASHRAF-1] Graphics Enhancement v1", GLUT_BITMAP_HELVETICA_12);
    
    char buffer[64];
    sprintf(buffer, "Lighting: %.2f | Glow: %s", gLighting.intensity, gLighting.enableGlow ? "ON" : "OFF");
...
static void keyboard(unsigned char key, int, int) {
    switch(key){
        ...
        case 'l': case 'L': gLighting.enableGlow = !gLighting.enableGlow; break;
    }
}
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Air Defense v1 - [ASHRAF-1] Graphics Enhancement");

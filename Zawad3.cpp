// Zawad3.cpp - Third Push (AI and Targeting System)
// Branch: feature/zawad-physics-engine-v3
// Contributor: Zawad
// Push #3 - Advanced AI Targeting System
...
// [ZAWAD MODIFICATION #3]
// Advanced AI Targeting with Priority System
struct TargetingAI {
    int   targetPriority;  // 0=none, 1=closest, 2=fastest, 3=threat-level
    float priorityScore;
    bool  intelligentTracking;
};
...
struct Drone {
    ...
    bool  detected;
    int   missileAssigned;
    float threatLevel;  // [ZAWAD#3] Threat evaluation
};
...
// [ZAWAD#3] AI targeting system
TargetingAI gTargeting = {1, 0.0f, true};
...
// [ZAWAD MODIFICATION #3] - Calculate drone threat level
static void calculateThreatLevel(Drone& drone, int index) {
    // [ZAWAD#3] Threat = (speed + proximity + altitude risk)
    drone.threatLevel = drone.speed * 0.5f;
    
    // Add proximity threat
    float distFromBase = fDist(drone.x, drone.y, 185.0f, 318.0f);
    drone.threatLevel += (1.0f - (distFromBase / RADAR_DETECT_RANGE)) * 0.3f;
    
    // Add altitude threat (lower = more dangerous)
    drone.threatLevel += (1.0f - (drone.y - 420.0f) / 180.0f) * 0.2f;
    
    if(drone.threatLevel > 1.0f) drone.threatLevel = 1.0f;
}

// [ZAWAD MODIFICATION #3] - Select best target based on AI priority
static int selectBestTarget() {
    int bestIdx = -1;
    float bestScore = -1.0f;
    
    for(int i = 0; i < MAX_DRONES; i++){
        if(!gDrones[i].active || !gDrones[i].detected) continue;
        if(gDrones[i].missileAssigned >= 0) continue;  // Already targeted
        
        float score = 0.0f;
        
        switch(gTargeting.targetPriority){
            case 1:  // Closest target
                score = 1.0f / (1.0f + fDist(gDrones[i].x, gDrones[i].y, 185.0f, 318.0f) / 100.0f);
                break;
            case 2:  // Fastest target
                score = gDrones[i].speed;
                break;
            case 3:  // Highest threat level
                score = gDrones[i].threatLevel;
                break;
            default:
                score = 0.5f;
        }
    ...
    gTargeting.priorityScore = bestScore;
    return bestIdx;
}
...
static void drawRadar() {
    ...
    for(int i = 0; i < MAX_DRONES; i++){
        if(gDrones[i].active && gDrones[i].detected){
            ...
            if(blipPulse > 0.3f){
                // [ZAWAD#3] Color based on threat level
                float threatColor = gDrones[i].threatLevel;
                col4(1.0f, 0.5f - threatColor * 0.25f, 0.05f, 1.0f);
...
static void initAll() {
    ...
    for(auto& d : gDrones) { 
        ...
        d.detected = false; 
        d.missileAssigned = -1;
        d.threatLevel = 0.0f;  // [ZAWAD#3] Initialize threat
    }
        ...
        for(int i = 0; i < MAX_DRONES; i++){
            ...
            if(!d.detected){
                ...
                if(dist < RADAR_DETECT_RANGE){
                    d.detected = true;
                    // [ZAWAD#3] Calculate threat level on detection
                    calculateThreatLevel(d, i);
                }
            } else {
                // [ZAWAD#3] Update threat level continuously
                calculateThreatLevel(d, i);
            }
        }

        // [ZAWAD#3] AI-based targeting: select best target and launch missile
        if(gFrameCounter % 30 == 0){  // Every 0.5 seconds
            int targetIdx = selectBestTarget();
            if(targetIdx >= 0){
                launchMissile(targetIdx);
            }
        }
...
static void display() {
    ...
    // [ZAWAD#3] Display AI targeting status
    col3(1.0f, 0.5f, 1.0f);
    drawText(12, 8, "[ZAWAD-3] AI Targeting System v3", GLUT_BITMAP_HELVETICA_12);
    
    char buffer[256];
    const char* priorityName[] = {"NONE", "CLOSEST", "FASTEST", "THREAT_LEVEL"};
    sprintf(buffer, "Mode: %s | Priority: %.2f | Score: %d", 
            priorityName[gTargeting.targetPriority], 
            gTargeting.priorityScore, gScore);
...
static void keyboard(unsigned char key, int, int) {
    switch(key){
        ...
        case '1': case '2': case '3': 
            gTargeting.targetPriority = key - '0'; break;
    }
}
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Air Defense v3 - [ZAWAD-3] AI Targeting");

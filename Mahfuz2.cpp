// Mahfuz2.cpp - Second Push (Enhanced Drone Detection)
// Branch: feature/mahfuz-defense-system-v1
// Contributor: Mahfuz
// Push #2 - Improved Drone Detection System
...
// [MAHFUZ MODIFICATION #2]
// Enhanced Drone Detection with Signal Strength Tracking
struct DetectionSignal {
    float strength;
    float angle;
    int   frameSinceDetection;
};
...
bool gPaused         = false;
int  gFrameCounter   = 0;
int  gDetectedCount  = 0;  // [MAHFUZ#2] Track detected drones
...
// [MAHFUZ MODIFICATION #2] - Enhanced signal strength calculation
static float calculateSignalStrength(float distance) {
    if(distance > RADAR_DETECT_RANGE) return 0.0f;
    return 1.0f - (distance / RADAR_DETECT_RANGE);
}

// [MAHFUZ MODIFICATION #2] - Enhanced Radar with signal strength display
static void drawRadar() {
    ...
    // [MAHFUZ#2] Enhanced drone detection with signal strength visualization
    gDetectedCount = 0;
    for(int i = 0; i < MAX_DRONES; i++){
        if(gDrones[i].active && gDrones[i].detected){
            gDrones[i].signal.frameSinceDetection++;
            ...
            // [MAHFUZ#2] Variable blip size based on signal strength
            float signalStrength = calculateSignalStrength(fDist(gDrones[i].x, gDrones[i].y, RADAR_X, RADAR_Y));
            float blipSize = 4.0f + signalStrength * 6.0f;
            
            float blipPulse = 0.5f + 0.5f * sinf(gFrameCounter * 0.15f);
            if(blipPulse > 0.3f){
                col4(1.0f, 0.25f, 0.05f, 1.0f);
                drawFilledCircle(cx + wx, cy + wy, blipSize);
                col4(1.0f, 0.60f, 0.30f, 0.6f);
                drawFilledCircle(cx + wx, cy + wy, blipSize + 3.0f);
            }
            gDetectedCount++;
        }
    }
...
static void initAll() {
    ...
    for(auto& d : gDrones) { 
        ...
        d.detected = false; 
        d.missileAssigned = -1;
        // [MAHFUZ#2] Initialize signal
        d.signal = {0.0f, 0.0f, 0};
    }
        ...
        for(int i = 0; i < MAX_DRONES; i++){
            ...
            if(!d.detected){
                ...
                if(dist < RADAR_DETECT_RANGE){
                    d.detected = true;
                    // [MAHFUZ#2] Record signal info
                    d.signal.strength = calculateSignalStrength(dist);
                    d.signal.frameSinceDetection = 0;
                }
            }
        }
    }
...
static void display() {
    ...
    // [MAHFUZ#2] Display detection statistics
    col3(0.0f, 1.0f, 0.0f);
    drawText(12, 8, "[MAHFUZ-2] Enhanced Detection v2", GLUT_BITMAP_HELVETICA_12);
    
    char buffer[64];
    sprintf(buffer, "Detected Targets: %d", gDetectedCount);
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Air Defense v2 - [MAHFUZ-2] Enhanced Detection");

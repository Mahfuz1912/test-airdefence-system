// Ashraf2.cpp - Second Push (Particle Effects Enhancement)
// Branch: feature/ashraf-graphics-enhancement-v2
// Contributor: Ashraf
// Push #2 - Advanced Particle System
...
// [ASHRAF#2] Particle system
ParticleEmitter gEmitter = {12.0f, 1.0f, true};
int  gParticlesGenerated = 0;
...
// [ASHRAF MODIFICATION #2] - Advanced particle spawning
static void spawnAdvancedParticles(float x, float y) {
    for(int i = 0; i < MAX_PARTICLES; i++){
        if(!gParticles[i].active){
            ...
            gParticles[i] = {x, y, cosf(a)*spd, sinf(a)*spd, gEmitter.lifespan, r, g, b, ptype, true};
            gParticlesGenerated++;
            static int cnt = 0;
            if(++cnt >= (int)gEmitter.emissionRate){ cnt = 0; return; }
        }
    }
}

// [ASHRAF MODIFICATION #2] - Enhanced particle rendering
static void drawAdvancedParticles() {
    glEnable(GL_BLEND);
...
// [ASHRAF MODIFICATION #2] - Enhanced particle physics
static void updateAdvancedParticles() {
    for(auto& p : gParticles){
        if(!p.active) continue;
        p.x += p.vx;
        p.y += p.vy;
        
        if(gEmitter.enablePhysics){
            if(p.type == 1){
                p.vy += 0.04f;
                p.vx *= 0.98f;
            } else {
        ...
        for(int i = 0; i < MAX_DRONES; i++){
            ...
            if(!d.detected){
                ...
                if(dist < RADAR_DETECT_RANGE){
                    d.detected = true;
                    spawnAdvancedParticles(d.x, d.y);  // [ASHRAF#2] Spawn particles
                }
            }
        }

        // [ASHRAF#2] Update particle physics
...
static void display() {
    ...
    // [ASHRAF#2] Display particle system status
    col3(0.0f, 1.0f, 1.0f);
    drawText(12, 8, "[ASHRAF-2] Advanced Particles v2", GLUT_BITMAP_HELVETICA_12);
    
    char buffer[128];
    sprintf(buffer, "Particles: %d | Emission: %.1f | Physics: %s", 
            gParticlesGenerated, gEmitter.emissionRate, 
            gEmitter.enablePhysics ? "ON" : "OFF");
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Air Defense v2 - [ASHRAF-2] Particle Effects");
